...
export class DynamicTerrain {
  ...
  private static _matZero: Float32Array = new Float32Array([
  ...
  constructor(
    ...
    this._precomputeInstances = options.precomputeInstances
      ? options.precomputeInstances
      : true;
    ...
    this._colorInstanceData =
      this._mapInstanceData && this._instanceColorData ? true : false;
    ...
    if (this._datamap) {
      this._mapNormals =
        options.mapNormals ||
        new Float32Array(this._mapSubX * this._mapSubZ * 3);
    } else {
      this._mapNormals = new Float32Array(
        this._terrainIdx * this._terrainIdx * 3
      );
    }
    this._mapQuads = [];
    ...
    this._mapSizeZ = Math.abs(
      mapData[(this._mapSubZ - 1) * this._mapSubX * 3 + 2] - mapData[2]
    );
    ...
    this._positions = this._terrain.getVerticesData(
    ...
    this._terrain.position.x =
      this._terrainCamera.globalPosition.x -
      this._terrainHalfSizeX +
      this.shiftFromCamera.x;
    this._terrain.position.z =
      this._terrainCamera.globalPosition.z -
      this._terrainHalfSizeZ +
      this.shiftFromCamera.z;
    // initialize deltaSub to make on the map
    const deltaNbSubX =
      (this._terrain.position.x - mapData[0]) / this._averageSubSizeX;
    const deltaNbSubZ =
      (this._terrain.position.z - mapData[2]) / this._averageSubSizeZ;
    this._deltaSubX =
  ...
  private _updateTerrain(updateSize: boolean): void {
    ...
    for (let j = 0 | 0; j <= terrainSub; j++) {
      ...
      for (let i = 0 | 0; i <= terrainSub; i++) {
        ...
        terIndex =
          mod(deltaSubZ + stepJ, terrainIdx) * terrainIdx +
          mod(deltaSubX + stepI, terrainIdx);
          ...
          if (quads[index]) {
            ...
            for (let t = 0; t < quad.length; t++) {
              ...
              if (partIndexes) {
                ...
                let min =
                  available < nbQuadParticles ? available : nbQuadParticles; // don't iterate beyond possible
                for (let pIdx = 0; pIdx < min; pIdx++) {
                  ...
                  pos.x =
                    x +
                    Math.floor((terrainPos.x - x - x0) / mapSizeX) * mapSizeX;
                  pos.y = data[idm + 1];
                  const z = data[idm + 2];
                  pos.z =
                    z +
                    Math.floor((terrainPos.z - z - z0) / mapSizeZ) * mapSizeZ;
                  rot.x = data[idm + 3];
                  rot.y = data[idm + 4];
                  rot.z = data[idm + 5];
                  scl.x = data[idm + 6];
                  ...
                  used = used + 1;
                  min =
                    available < nbQuadParticles ? available : nbQuadParticles;
                }
          ...
          if (quads[index]) {
            ...
            for (let t = 0; t < quad.length; t++) {
              ...
              if (instanceIndexes && instancedBuffer) {
                ...
                var used = rem > 0 ? rem : 0;
                let min =
                  available < nbQuadInstances ? available : nbQuadInstances; // don't iterate beyond possible
                for (let iIdx = 0; iIdx < min; iIdx++) {
                  ...
                  used = used + 1;
                  min =
                    available < nbQuadInstances ? available : nbQuadInstances;
                }
                available = available > 0 ? available : 0;
  ...
  public static GetHeightFromMap(
    ...
    const mapSizeZ = Math.abs(
      mapData[(mapSubZ - 1) * mapSubX * 3 + 2] - mapData[2]
    );
    return DynamicTerrain._GetHeightFromMap(
      x,
      z,
  ...
  public contains(x: number, z: number): boolean {
    ...
    const terrainIdx = this._terrainIdx;
    if (
      x < positions[0] + meshPosition.x ||
      x > positions[3 * terrainIdx] + meshPosition.x
    ) {
      return false;
    }
    if (
  ...
  public static CreateMapFromHeightMap(
    ...
    DynamicTerrain.CreateMapFromHeightMapToRef(
      heightmapURL,
      options,
      data,
      scene
  ...
  public static CreateMapFromHeightMapToRef(
    ...
    const onload = (img: CanvasImageSource) => {
      ...
      const buffer = context!!.getImageData(0, 0, bufferWidth, bufferHeight)
      ...
      for (let row = 0; row < subZ; row++) {
        for (let col = 0; col < subX; col++) {
          ...
          const heightmapX =
            (((x + width * 0.5) / width) * (bufferWidth - 1)) | 0;
          const heightmapY =
            (bufferHeight -
              1 -
              ((z + height * 0.5) / height) * (bufferHeight - 1)) |
            0;
          const pos = (heightmapX + heightmapY * bufferWidth) * 4;
          const gradient =
            (buffer[pos] * filter.r +
              buffer[pos + 1] * filter.g +
  ...
  private static _CopyArrayValuesFromToRef(
    ...
    start: number,
    nb: number,
    target: Float32Array | Array<number>
  ) {
    for (let i = 0; i < nb; i++) {
